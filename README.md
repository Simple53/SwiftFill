...
- 修复弹窗 iframe 无法一键填充的问题（支持多层 iframe、动态新增 iframe、延迟注入内容）。
- 优化 iframe 穿透策略：递归广播 + load 监听 + 多次重试，提升政务站点兼容性。
- 优化悬浮助手视觉和交互细节，保持原有使用方式不变。
- 扩展图标改为扁平化设计，重绘并输出真实尺寸 `16/32/48/128`，提升工具栏显示清晰度。

## 历史版本 Change Log

- `1.0-beta`：首个公开测试版，提供基础表单自动填充和悬浮助手能力。
- `v2.0`：增强弹窗 iframe 一键填充能力，优化图标和视觉一致性。
